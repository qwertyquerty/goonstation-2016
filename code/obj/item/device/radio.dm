...
/obj/item/device/radio/talk_into(mob/M as mob, messages, secure, real_name, lang_id)
	// According to a pair of DEBUG_MESSAGE calls set up for testing, no radio jammer check for the src radio was performed.
...
/obj/item/device/radio/proc/accept_rad(obj/item/device/radio/R as obj, message, var/datum/radio_frequency/freq)
	if (message)
		...
		// Secure channel lookup when R.frequency != src.frequency. According to DEBUG_MESSAGE calls set up for testing,
